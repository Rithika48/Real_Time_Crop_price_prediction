...
def load_resources():
    ...
    try:
        model_data = joblib.load('final_crop_model.pkl')
...
def run_scraper():
    ...
    try:
        ...
        scraper_script = 'karnataka_crop_scraper.py'
        ...
        # Helper to extract JSON data from the scraper's text output
        def extract_json_data(text):
            try:
                json_part = text.split("---JSON_DATA_START---")[1].split("---JSON_DATA_END---")[0]
                return json.loads(json_part.strip())
            except (IndexError, json.JSONDecodeError):
                return []

        # Check the output for different success messages
        if "Data successfully saved to" in output:
            # Reload the app's data to reflect the changes
            global model, encoders, df # Reload resources
            model, encoders, df = load_resources() 
            logging.info("Data resources reloaded after successful scrape.")
            scraped_data = extract_json_data(output)
            return jsonify({'success': True, 'message': 'Successfully collected new data from the source.', 'data': scraped_data})
        elif "No recent crop rate updates found" in output:
            return jsonify({'success': True, 'message': 'No new data to fetch. Your records are already up-to-date.', 'data': []})
        elif "Generating dummy data" in output:
            # This handles the case where live scraping fails but we have demo data
            scraped_data = extract_json_data(output)
            return jsonify({'success': True, 'message': 'Live scraping failed, but demonstration data was prepared.', 'data': scraped_data})
        else:
            return jsonify({'success': False, 'message': 'Scraper ran, but the outcome was unclear.', 'data': []})

    except FileNotFoundError:
        logging.error("Scraper script 'karnataka_crop_scraper.py' not found.")
        return jsonify({'success': False, 'message': 'Error: Scraper script not found.'}), 500
    except subprocess.CalledProcessError as e:
        logging.error(f"Scraper script failed with error:\n{e.stderr}")
        return jsonify({'success': False, 'message': f'Scraper failed: {e.stderr}'}), 500
...
def recommend_crop():
    try:
        ...
        # Calculate profit from actual price data or model predictions
...
def api_calculate_profit():
    ...
    try:
        ...
        area_acres = float(data.get('area', 1)) # Get area from request
        ...
        return jsonify({
            ...
            'profit': profit,
            'roi': roi,
            'avgPrice': predicted_price, # Frontend expects 'avgPrice'
            'area': area_acres,
